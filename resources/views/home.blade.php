    ...
    @section('hero')
        <div class="w-full text-center py-32">
            <h1 class="text-2xl md:text-3xl font-bold text-center lg:text-5xl text-gray-700">
                Welcome to <span class="text-yellow-500">&lt;Aleksandar&gt;</span> <span class="text-gray-900"> News</span>
            </h1>
            ...
            <div class="w-full">
                <div class="grid grid-cols-3 gap-10 w-full">
                    @foreach($featured as $fPost)
                        ...
                        <x-posts.post-card :post="$fPost"></x-posts.post-card>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="w-full mb-5">
            <div class="grid grid-cols-3 gap-10 gap-y-32 w-full">
                @foreach($latest as $lPost)
                    <div class="md:col-span-1 col-span-3">
                        <x-posts.post-card :post="$lPost"></x-posts.post-card>
                    </div>
                @endforeach
            </div>
        </div>
